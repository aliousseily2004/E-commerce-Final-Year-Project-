...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $singularSearchTerm = rtrim($searchTerm, 's');

    // Update the base query to include the search term with OR for singular/plural
    $baseQuery .= " AND (p.title LIKE :searchTerm OR p.title LIKE :singularSearchTerm)";
    $countQuery .= " AND (p.title LIKE :searchTerm OR p.title LIKE :singularSearchTerm)";

    // Add the search terms to the parameters
    $params[':searchTerm'] = "%{$searchTerm}%";
    $params[':singularSearchTerm'] = "%{$singularSearchTerm}%";
}
